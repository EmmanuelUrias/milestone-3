...
const ExpenseFeed = (smallScreen: any) => {
    ...
    const newExpense = async (event: any) => {
      ...
      const newExpenseRes = await fetch(`http://budgetbuddy-env.eba-hjjiskth.us-east-1.elasticbeanstalk.com/expense/${user.user_id}`, {
        method: 'POST',
        headers: {
    ...
    const getExpenses = async () => {
      const allExpenses = await fetch(`http://budgetbuddy-env.eba-hjjiskth.us-east-1.elasticbeanstalk.com/expense/${user.user_id}`, {
        method: 'GET',
        headers: {
          Authorization: `The chosen one ${token}`
        }
